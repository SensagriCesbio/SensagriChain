...
=========================================================================*/
#ifndef __RandomForestsMachineLearningModel_h
#define __RandomForestsMachineLearningModel_h
...
class myCvRTreesWrapper;
namespace otb
{
  template <class TInputValue, class TTargetValue>
class ITK_EXPORT myRandomForestsMachineLearningModel
  : public myMachineLearningModel <TInputValue, TTargetValue>
{
public:
  /** Standard class typedefs. */
  typedef myRandomForestsMachineLearningModel               Self;
  typedef myMachineLearningModel<TInputValue, TTargetValue> Superclass;
  ...
  typedef typename Superclass::ProbaVectorType              ProbaVectorType;
  typedef typename Superclass::MapType                      MapType;
  // Other
  typedef itk::VariableSizeMatrix<float>                    VariableImportanceMatrixType;
  

  //opencv typedef
  typedef myCvRTreesWrapper RFType;
  
  /** Run-time type information (and related methods). */
  itkNewMacro(Self);
  itkTypeMacro(myRandomForestsMachineLearningModel, myMachineLearningModel);

  /** Train the machine learning model */
  virtual void Train();
  /** Predict values using the model */
  virtual TargetSampleType Predict(const InputSampleType& input, ConfidenceValueType *quality=NULL, ProbaVectorType *votes=NULL) const;
  ...
  virtual void GetLabels(std::vector<unsigned int>* labels);

  virtual void SetLabelMap(MapType LabelMap);

  virtual std::vector<float> GetOOBErrors();
  virtual void SetOOBErrors(std::vector<float> oob_array);

  virtual std::vector< std::vector<float> > GetOOBVotes();
  virtual void SetOOBVotes(std::vector< std::vector<float> > oob_array);
...
protected:
  ...
  /** Destructor */
  virtual ~myRandomForestsMachineLearningModel();
...
private:
  myRandomForestsMachineLearningModel(const Self &); //purposely not implemented
  void operator =(const Self&); //purposely not implemented

  myCvRTreesWrapper * m_RFModel;
...
#include "RandomForestsMachineLearningModel.txx"
#endif

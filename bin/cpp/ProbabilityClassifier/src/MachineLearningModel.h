...
=========================================================================*/
#ifndef __MachineLearningModel_h
#define __MachineLearningModel_h
...
class ITK_EXPORT myMachineLearningModel
  : public itk::Object
{
public:
  /**\name Standard ITK typedefs */
  //@{
  typedef myMachineLearningModel                          Self;
  ...
  typedef itk::VariableLengthVector<double>             ProbaVectorType;

  typedef std::map<unsigned int, unsigned int>          MapType;
  /**\name Standard macros */
  //@{
  /** Run-time type information (and related methods). */
  itkTypeMacro(myMachineLearningModel, itk::Object);
  //@}
  
  itkGetMacro(ComputeProba, bool);
  itkSetMacro(ComputeProba, bool);
  
  itkGetMacro(UseLabelMap, bool);
  itkSetMacro(UseLabelMap, bool);

  virtual void SetLabelMap(MapType LabelMap) = 0;
  virtual void SetOOBErrors(std::vector<float> oob_array) = 0;
  virtual std::vector<float> GetOOBErrors() = 0;
  virtual void SetOOBVotes(std::vector< std::vector<float> > oob_array) = 0;
  virtual std::vector< std::vector<float> > GetOOBVotes() = 0;
  
  /** Train the machine learning model */
  virtual void Train() =0;

  /** Predict values using the model */
  virtual TargetSampleType Predict(const InputSampleType& input, ConfidenceValueType *quality = NULL, ProbaVectorType *votes = NULL) const = 0;
  ...
  virtual unsigned int GetNumberOfClasses() = 0;
  virtual void GetLabels(std::vector<unsigned int> * labels) = 0;
  ...
  //@}
  //unsigned int nbClasses = this->nbClasses; 
protected:
  /** Constructor */
  myMachineLearningModel();

  /** Destructor */
  virtual ~myMachineLearningModel();
  ...
  bool m_UseLabelMap;
private:
  myMachineLearningModel(const Self &); //purposely not implemented
...
#include "MachineLearningModel.txx"
#endif
